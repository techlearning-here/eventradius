...
try:
    ...
    print("Make sure you're running this script with the virtual environment activated:")
...
async def test_events_table_access(client: Client) -> Dict[str, Any]:
    ...
    try:
        ...
        if create_response.data:
            ...
            update_response = client.table('events').update(update_data).eq('id', event_id).execute()
        ...
        if 'created_by' in error_msg and 'not-null constraint' in error_msg:
            print(f"    ✅ CREATE operation correctly blocked due to authentication requirement")
