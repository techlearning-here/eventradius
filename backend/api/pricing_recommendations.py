...
router = APIRouter(
    prefix="/api/pricing/recommendations", tags=["pricing-recommendations"]
)
...
class ApproveRecommendationRequest(BaseModel):
    max_uses: int = Field(
        ..., ge=1, le=1000, description="Maximum number of times the code can be used"
    )


class RejectRecommendationRequest(BaseModel):
    reason: Optional[str] = Field(
        None, max_length=500, description="Optional reason for rejection"
    )
...
async def list_recommendations(
    ...
    recommendation_type: Optional[str] = Query(
        None, description="Filter by type: ai, rule_based"
    ),
...
async def approve_recommendation(
    ...
    events = fetch_records(
        "events", filters={"id": recommendation["event_id"]}, limit=1
    )
    if not events:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    existing_codes = fetch_records(
        "promo_codes", filters={"event_id": recommendation["event_id"]}
    )
    ...
    discount_amount = Decimal(pricing_rule["base_price"]) - Decimal(
    ...
    # Merge original recommendation with updated fields
    merged_result = {**recommendation, **result}
    response = RecommendationResponse(**merged_result)
    # Merge promo_data with insert result for complete promo code response
    merged_promo_result = {**promo_data, **promo_result}
    response.promo_code = PromoCodeResponse(**merged_promo_result)
    return response
...
async def reject_recommendation(
    ...
    # Merge original recommendation with updated fields
    merged_result = {**recommendation, **result}
    return RecommendationResponse(**merged_result)
...
async def generate_rule_based_recommendation(
    ...
    logger.info(
        f"Event ID: {event_id}, Current time: {now.isoformat()}, Event start time: {event_start}"
    )
    ...
    if event_start:
        try:
            if isinstance(event_start, str):
                ...
                logger.info(
                    f"Calculated hours_before: {hours_before} (event_time: {event_time.isoformat()})"
                )
        except (ValueError, TypeError) as e:
            logger.error(f"Failed to parse event start time: {e}")
            pass
    ...
    # Get event capacity
    max_capacity = event.get("max_participants") or pricing_rule.get(
        "max_capacity", 100
    )
    if max_capacity > 0:
        current_participants = event.get("current_participants", 0)
        occupancy_percent = max(
            occupancy_percent, (current_participants / max_capacity) * 100
        )
    ...
    for rule in all_rules:
        ...
        if (
            occupancy_percent < rule["occupancy_threshold"]
            and hours_before <= threshold_hours
        ):
