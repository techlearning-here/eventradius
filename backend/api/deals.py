...
from config.database import call_rpc, fetch_records, fetch_single_record, insert_record
...
async def get_event_deal(
    ...
    if discount_amount:
        ...
        discounted_price = original_price * (
            Decimal(1) - Decimal(promo_code["discount_percent"]) / Decimal(100)
        )
...
async def list_deals(
    ...
    try:
        ...
        logger.warning(
            f"RPC get_active_deals failed: {e}, falling back to manual query"
        )
...
async def claim_deal(
    ...
    try:
        ...
        # Note: update_record function needs to be imported from config.database
