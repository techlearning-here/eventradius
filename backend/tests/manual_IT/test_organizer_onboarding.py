...
class TestOrganizerOnboardingWorkflow:
    ...
    def test_organizer_onboarding_complete_workflow(self, api_client):
        ...
        try:
            ...
            assert response.status_code == 200, f"Failed to get organizer status: {response.text}"
            ...
            if initial_status.get("is_organizer") and initial_status.get("is_active"):
                print("  ✅ User is already an active organizer, testing event creation...")
            ...
            assert response.status_code == 200, f"Failed to get verification status: {response.text}"
            ...
            response = api_client.post(
                f"{BASE_URL}/api/verification/email/send",
                json=email_data
            )
            assert response.status_code == 200, f"Failed to send email verification: {response.text}"
            ...
            if email_token:
                ...
                assert response.status_code == 200, f"Failed to verify email: {response.text}"
            ...
            response = api_client.post(
                f"{BASE_URL}/api/verification/phone/send",
                json=phone_verify_data
            )
            assert response.status_code == 200, f"Failed to send phone verification: {response.text}"
            ...
            if phone_token:
                ...
                assert response.status_code == 200, f"Failed to verify phone: {response.text}"
    ...
    def _get_user_email(self, api_client) -> str:
        ...
        if response.status_code == 200:
            data = response.json()
            return data.get("email", f"test_{datetime.now().strftime('%Y%m%d%H%M%S')}@example.com")
    ...
    def test_organizer_activation_check(self, api_client):
        ...
        try:
            ...
            assert response.status_code == 200, f"Activation check failed: {response.text}"
    ...
    def test_verification_status_endpoint(self, api_client):
        ...
        try:
            ...
            required_fields = [
                "is_organizer", "email_verified", "phone_verified",
                "phone_provided", "email_verification_sent", "phone_verification_sent",
                "organizer_status", "is_active", "can_create_events",
                "missing_requirements", "next_actions"
            ]
