...
interface BasicInfoProps {
  ...
  onEventStatusChange: (value: 'draft' | 'published' | 'cancelled') => void;
}
  ...
  return (
    <div>
      ...
      <div className="space-y-8 md:space-y-12">
        ...
        {/* Ticket Pricing Description (shown only for paid events) */}
        {isPaidEvent && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-8 h-8 bg-blue-500 text-white rounded-full flex items-center justify-center font-semibold">
              ...
              <h3 className="text-lg font-semibold">Ticket Pricing Details</h3>
            </div>
            <p className="text-white mb-4">Describe your ticket pricing structure for attendees</p>
            <div className="space-y-3">
              <label className="block text-sm font-medium text-white mb-2">
                Pricing Description <span className="text-gray-400">(Optional)</span>
              </label>
              <textarea
                placeholder="Adults: $25&#10;Children (4-12): $15&#10;Family Pack (2 adults + 2 kids): $70"
                className="w-full text-black text-base leading-relaxed focus:outline-none bg-white border border-gray-300 rounded-md p-3 placeholder:text-gray-400 hover:border-blue-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all resize-y min-h-[80px]"
                rows={3}
                value={ticketPricingDescription}
                onChange={(e) => onTicketPricingDescriptionChange(e.target.value)}
              />
              <p className="text-xs text-white">
                Provide details about different ticket types, pricing tiers, and what's included. This information will be displayed on the event page.
              </p>
            </div>
          </div>
        )}
